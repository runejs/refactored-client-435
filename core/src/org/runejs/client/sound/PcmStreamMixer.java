...
public class PcmStreamMixer extends PcmStream {
    ...
    public synchronized void skip(int arg0) {
        do {
            ...
            this.anInt2877 += i;
            this.loop0();
            PcmStreamMixerListener class40_sub8 = (PcmStreamMixerListener) this.head.peekFirst();
    ...
    private int fill0(int[] arg0, int arg1, int arg2) {
        ...
        if(this.anInt2875 <= 0) {
            ...
            for(int i = 0; i < 8; i++) {
                ...
                for(PcmStream class40_sub9 = (PcmStream) linkedList.peekFirst(); class40_sub9 != null; class40_sub9 = (PcmStream) linkedList.pollFirst()) {
                    int i_1_ = method849(class40_sub9);
                    if(i_1_ != i)
                        this.substreams[i_1_].addFirst(class40_sub9);
                }
            }
        }
        for(int i = 0; i < 8; i++) {
            LinkedList linkedList = this.substreams[i];
            for(PcmStream stream = (PcmStream) linkedList.peekFirst(); stream != null; stream = (PcmStream) linkedList.pollFirst()) {
                stream.active = false;
    ...
    private void loop0() {
        if(this.anInt2877 > 0) {
            for(PcmStreamMixerListener class40_sub8 = (PcmStreamMixerListener) this.head.peekFirst(); class40_sub8 != null; class40_sub8 = (PcmStreamMixerListener) this.head.pollFirst())
