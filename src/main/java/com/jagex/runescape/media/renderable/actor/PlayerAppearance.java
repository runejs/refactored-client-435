...
public class PlayerAppearance {
    ...
    public static int[] playerSkinColors = new int[]{
    ...
    public static void method381(Actor arg1) {
        if(MovedStatics.pulseCycle == arg1.anInt3107 || arg1.playingAnimation == -1 ||
                arg1.playingAnimationDelay != 0 ||
                arg1.anInt3115 + 1 > ProducingGraphicsBuffer_Sub1.getAnimationSequence(
                        arg1.playingAnimation).frameLengths[arg1.anInt3104]) {
    ...
    public int getHeadModelId() {
        ...
        return appearance[1] + (appearance[11] << 5) + (appearanceColors[4] << 20) + (appearanceColors[0] << 25) +
                (appearance[0] << 15) + (appearance[8] << 10);
    }
    ...
    public Model getAnimatedModel(
            AnimationSequence animation1, AnimationSequence animation2, int unknown1, int unknown2
    ) {
        if(transformationNpcId != -1) {
            return ActorDefinition.getDefinition(transformationNpcId).getChildModel(
                    animation1, animation2, unknown1, unknown2);
        }
        ...
        if(cachedModel == null) {
            ...
            for(int bodyPart = 0; bodyPart < 12; bodyPart++) {
                ...
                if(appearanceModel >= 256 && appearanceModel < 512 && !IdentityKit.cache(-256 + appearanceModel)
                        .isBodyModelCached()) {
                    invalid = true;
                }
    ...
    public void loadCachedAppearance(int unknown1, boolean unknown2) {
        if(unknown1 != 1 || !gender) {
            ...
            if(i != 0) {
                ...
                } while(identityKit == null || identityKit.nonSelectable ||
                        identityKit.bodyPartId != unknown1 + (!gender ? 0 : 7));
