...
public class ScreenController {
    ...
    public static void frameMode(ScreenMode screenMode) {
        if(frameMode != screenMode) {
            ...
            if(screenMode == ScreenMode.FIXED) {
                frameWidth = 765;
                frameHeight = 540;
                Class35.gameFrame.setResizable(false);
                Class35.gameFrame.setPreferredSize(
                        new Dimension(ScreenController.frameWidth, ScreenController.frameHeight));
                Class35.gameFrame.setMinimumSize(
                        new Dimension(ScreenController.frameWidth, ScreenController.frameHeight));
            } else if(screenMode == ScreenMode.RESIZABLE) {
                frameWidth = 900;
                frameHeight = 637;
                Class35.gameFrame.setResizable(true);
                Class35.gameFrame.setPreferredSize(
                        new Dimension(ScreenController.frameWidth, ScreenController.frameHeight));
                Class35.gameFrame.setMinimumSize(
                        new Dimension(ScreenController.frameWidth, ScreenController.frameHeight));
            ...
            Class35.gameFrame.setSize(ScreenController.frameWidth, ScreenController.frameHeight);
            Dimension innerSize = getInnerSize(Class35.gameFrame);
    ...
    public static void refreshFrameSize() {
        ...
        if(frameMode != ScreenMode.FIXED) {
            if(frameWidth != Class35.gameFrame.getWidth()) {
                frameWidth = Class35.gameFrame.getWidth();
                setBounds();
            }
            if(frameHeight != Class35.gameFrame.getHeight()) {
                frameHeight = Class35.gameFrame.getHeight();
                setBounds();
            }
        }
    }
    ...
    public static void setBounds() {
        ...
        drawHeight = innerSize.height;
        drawWidth = innerSize.width;
        Rasterizer3D.prepare(null, frameMode == ScreenMode.FIXED ? 512 : drawWidth,
                frameMode == ScreenMode.FIXED ? 334 : drawHeight
        );
        Class24.fullScreenTextureArray = Rasterizer3D.setLineOffsets(null);
        Rasterizer3D.prepare(null, frameMode == ScreenMode.FIXED ? 479 : drawWidth,
                frameMode == ScreenMode.FIXED ? 96 : drawHeight
        );
        MovedStatics.chatboxLineOffsets = Rasterizer3D.setLineOffsets(null);
        Rasterizer3D.prepare(null, frameMode == ScreenMode.FIXED ? 190 : drawWidth,
                frameMode == ScreenMode.FIXED ? 261 : drawHeight
        );
        ActorDefinition.sidebarOffsets = Rasterizer3D.setLineOffsets(null);
        Rasterizer3D.prepare(null, frameMode == ScreenMode.FIXED ? 765 : drawWidth,
                frameMode == ScreenMode.FIXED ? 503 : drawHeight
        );
        ...
        Scene.method95(500, 800, frameMode == ScreenMode.FIXED ? 512 : drawWidth,
                frameMode == ScreenMode.FIXED ? 334 : drawHeight, is
        );
        MouseHandler.gameCanvas.setSize(
        ...
        if(Class51.currentAction <= 35 && Class51.currentAction >= 30) {
            VarbitDefinition.gameScreenImageProducer = Class40_Sub5_Sub13.createGraphicsBuffer(
                    ScreenController.frameMode == ScreenMode.FIXED ? 512 : ScreenController.drawWidth,
                    ScreenController.frameMode == ScreenMode.FIXED ? 334 : ScreenController.drawHeight,
                    Class35.gameFrame
            );
    ...
    public static void RenderResizableUI() {
        ...
        if(DebugView) {
            ...
            if(isCoordinatesInExtendedTabArea(mX, mY) && !isCoordinatesInTabArea(mX, mY)) {
                Rasterizer.drawFilledRectangleAlpha(tabInterFaceTop[0], tabInterFaceTop[1], tabInterFaceTop[2],
                        tabInterFaceTop[3], 0x00FF00, 90
                );
                Rasterizer.drawFilledRectangleAlpha(tabInterFaceBottom[0], tabInterFaceBottom[1], tabInterFaceBottom[2],
                        tabInterFaceBottom[3], 0x00FF00, 90
                );
            } else {
                Rasterizer.drawFilledRectangleAlpha(tabInterFaceTop[0], tabInterFaceTop[1], tabInterFaceTop[2],
                        tabInterFaceTop[3], 0x0000FF, 90
                );
                Rasterizer.drawFilledRectangleAlpha(tabInterFaceBottom[0], tabInterFaceBottom[1], tabInterFaceBottom[2],
                        tabInterFaceBottom[3], 0x0000FF, 90
                );
            }
            if(isCoordinatesInTabArea(mX, mY)) {
                Rasterizer.drawFilledRectangleAlpha(tabInterFaceCoords[0], tabInterFaceCoords[1],
                        MovedStatics.tabImageProducer.width, MovedStatics.tabImageProducer.height, 0x00FF00, 90
                );
            } else {
                Rasterizer.drawFilledRectangleAlpha(tabInterFaceCoords[0], tabInterFaceCoords[1],
                        MovedStatics.tabImageProducer.width, MovedStatics.tabImageProducer.height, 0x0000FF, 90
                );
            }
            ...
            if(isCoordinatesInChatArea(mX, mY)) {
                Rasterizer.drawFilledRectangleAlpha(17, drawHeight - (162) + 16, RSCanvas.chatBoxImageProducer.width,
                        RSCanvas.chatBoxImageProducer.height, 0x00FF00, 90
                );
            } else {
                Rasterizer.drawFilledRectangleAlpha(17, drawHeight - (162) + 16, RSCanvas.chatBoxImageProducer.width,
                        RSCanvas.chatBoxImageProducer.height, 0x0000FF, 90
                );
            }
    ...
    public static void RenderTabArea(int x, int y) {
        ...
        drawFramePieceCutout(MovedStatics.tabPieceUpperRight, x, y + 39, MovedStatics.tabPieceUpperRight.width - 15,
                MovedStatics.tabPieceUpperRight.height, 15, 0
        );
        drawFramePieceCutout(PlayerAppearance.tabPieveLowerRight, x, y + 191,
                PlayerAppearance.tabPieveLowerRight.width - 35, PlayerAppearance.tabPieveLowerRight.height, 35, 0
        );
        drawFramePieceCutout(
                Class17.chatboxTop, x, y + 172, Class17.chatboxTop.width - 531, Class17.chatboxTop.height, 531, 0);
        drawFramePieceCutout(
                RSCanvas.tabBottom, x, y + 300, RSCanvas.tabBottom.width - 35, RSCanvas.tabBottom.height, 35, 0);
        drawFramePieceCutout(InteractiveObject.tabTop, x, y, InteractiveObject.tabTop.width - 15,
                InteractiveObject.tabTop.height - 6, 15, 6
        );
    ...
    public static void RenderChatArea(int x, int y) {
        ...
        drawFramePieceCutout(
                Class17.chatboxTop, x, y, Class17.chatboxTop.width - 37, Class17.chatboxTop.height - 3, 0, 3);
        ...
        drawFramePieceCutout(PlayerAppearance.tabPieveLowerRight, x + 496, y + 16, 20,
                PlayerAppearance.tabPieveLowerRight.height, 0, 0
        );
    ...
    public static void drawFramePieceCutout(
            ProducingGraphicsBuffer framePiece, int x, int y, int width, int height, int offsetX, int offsetY
    ) {
        Rasterizer.copyPixelsCutOff(framePiece.pixels, framePiece.width, framePiece.height, x, y, width, height,
                offsetX, offsetY
        );
    }
    ...
    public static boolean isCoordinatesIn3dScreen(int x, int y) {
        if(frameMode == ScreenMode.FIXED) {
            ...
            return x > 0 && y > 0 && x < drawWidth && y < drawHeight && !isCoordinatesInExtendedTabArea(x, y) &&
                    !isCoordinatesInTabArea(x, y) && !isCoordinatesInMinimapArea(x, y) &&
                    !isCoordinatesInExtendedChatArea(x, y);
        ...
        if(frameMode == ScreenMode.FIXED) {
            if(GameInterface.tabAreaInterfaceId != -1) {
                ...
                Class13.handleInterfaceActions(
                        1, mouseX, mouseY, 553, 205, 743, 466, Player.tabWidgetIds[Player.currentTabId]);
            }
        } else {
            int[] tabInterFaceCoords = tabProducer.getTabInterfaceCoordSize(drawWidth - 241, drawHeight - (334));

            if(GameInterface.tabAreaInterfaceId != -1) {
                Class13.handleInterfaceActions(1, mouseX, mouseY, tabInterFaceCoords[0], tabInterFaceCoords[1],
                        tabInterFaceCoords[0] + tabInterFaceCoords[2], tabInterFaceCoords[1] + tabInterFaceCoords[3],
                        GameInterface.tabAreaInterfaceId
                );
            } else if(Player.tabWidgetIds[Player.currentTabId] != -1) {
                Class13.handleInterfaceActions(1, mouseX, mouseY, tabInterFaceCoords[0], tabInterFaceCoords[1],
                        tabInterFaceCoords[0] + tabInterFaceCoords[2], tabInterFaceCoords[1] + tabInterFaceCoords[3],
                        Player.tabWidgetIds[Player.currentTabId]
                );
            }
        }
    }
        ...
        if(frameMode == ScreenMode.FIXED) {
            if(GameInterface.chatboxInterfaceId == -1) {
                if(ChatBox.dialogueId == -1) {
                    if(y < 434 && x < 426) {
                        ClientScriptRunner.method873(y - 357);
                    }
                } else {
            ...
            if(GameInterface.chatboxInterfaceId == -1) {
                if(ChatBox.dialogueId == -1) {
                    if(y < frameHeight - (112) && x < 426) {
                        ClientScriptRunner.method873(y - (frameHeight - 162));
                    }
                } else {
                    Class13.handleInterfaceActions(
                            3, x, y, 17, (frameHeight - 162), frameWidth - 496, frameHeight - 54, ChatBox.dialogueId);
                }
            } else {
                Class13.handleInterfaceActions(2, x, y, 17, (frameHeight - 162), frameWidth - 496, frameHeight - 54,
                        GameInterface.chatboxInterfaceId
                );
            }
        }
    ...
    public static void handleMinimapMouse() {
        ...
        if(MouseHandler.clickType == 1) {
            ...
            if(frameMode == ScreenMode.FIXED) {
                ...
                x = -(drawWidth - 210) + Class57.clickX;
                y = -5 + RSString.clickY - 4;
            ...
            if(Item.obfuscatedKeyStatus[81] && InteractiveObject.playerRights > 1) {
                ...
                SceneCluster.packetBuffer.putString(MessageFormat
                        .format(" move {0} {1}", Integer.toString(destX + SpotAnimDefinition.baseX),
                                Integer.toString(destY + Class26.baseY)
                        ));
            } else {
                boolean bool = Pathfinding.doWalkTo(0, 0, Player.localPlayer.pathY[0], destX, 0, true, 0, 0,
                        Player.localPlayer.pathX[0], destY, 1
                );
                if(bool) {
    ...
    public static void handleTabClick() {
        ...
        if(MouseHandler.clickType == 1) {
            ...
            } else if(isCoordinatesInExtendedTabArea(x, y)) {
