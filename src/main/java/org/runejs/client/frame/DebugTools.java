    ...
    public static void drawWalkPath() {
        ...
        for (int i = 1; i < walkpathX.length; i++) {
            ...
            if (lastTilePos != null && nextPos != null) {
                ScreenController.gameRasterizer.drawDiagonalLine(lastTilePos.x, lastTilePos.y, nextPos.x, nextPos.y, 0x00FFAC);
            }
        ...
        if (pathStartPos != null) {
            ScreenController.gameRasterizer.drawCircle(pathStartPos.x, pathStartPos.y, 2, 0xE055DE);
        ...
        if (pathFinishPos != null) {
            ScreenController.gameRasterizer.drawCircle(pathFinishPos.x, pathFinishPos.y, 4, 0xE055DE);
            ...
            for (int y = Math.max(0, tileY - clippingRenderDistance); y < Math.min(104, tileY + clippingRenderDistance); y++) {
                ...
                if ((data & 0x2) == 0x2) { // north
                    if (posNE != null && posNW != null) {
                        ScreenController.gameRasterizer.drawDiagonalLine(posNE.x, posNE.y, posNW.x, posNW.y, blockWalkColor);
                    }
                }

                if ((data & 0x8) == 0x8) { // east
                    if (posSE != null && posNE != null) {
                        ScreenController.gameRasterizer.drawDiagonalLine(posSE.x, posSE.y, posNE.x, posNE.y, blockWalkColor);
                    }
                }

                if ((data & 0x20) == 0x20) { // south
                    if (posSE != null && posSW != null) {
                        ScreenController.gameRasterizer.drawDiagonalLine(posSE.x, posSE.y, posSW.x, posSW.y, blockWalkColor);
                    }
                }

                if ((data & 0x80) == 0x80) { // west
                    if (posSW != null && posNW != null) {
                        ScreenController.gameRasterizer.drawDiagonalLine(posSW.x, posSW.y, posNW.x, posNW.y, blockWalkColor);
                    }
                }
                ...
                if ((data & 0x400) == 0x400) { // north (projectile)
                    if (posNEA != null && posNWA != null) {
                        ScreenController.gameRasterizer.drawDiagonalLine(posNEA.x, posNEA.y, posNWA.x, posNWA.y, blockProjectileColor);

                        if (posNE != null) {
                            ScreenController.gameRasterizer.drawDiagonalLine(posNE.x, posNE.y, posNEA.x, posNEA.y, blockProjectileColor);
                        }

                        if (posNW != null) {
                            ScreenController.gameRasterizer.drawDiagonalLine(posNW.x, posNW.y, posNWA.x, posNWA.y, blockProjectileColor);
                        }
                    }
                }
                ...
                if ((data & 0x1000) == 0x1000) { // east (projectile)
                    if (posSEA != null && posNEA != null) {
                        ScreenController.gameRasterizer.drawDiagonalLine(posSEA.x, posSEA.y, posNEA.x, posNEA.y, blockProjectileColor);
                    }
                }

                if ((data & 0x4000) == 0x4000) { // south (projectile)
                    if (posSEA != null && posSWA != null) {
                        ScreenController.gameRasterizer.drawDiagonalLine(posSEA.x, posSEA.y, posSWA.x, posSWA.y, blockProjectileColor);
                    }
                }

                if ((data & 0x10000) == 0x10000) { // west (projectile)
                    if (posSWA != null && posNWA != null) {
                        ScreenController.gameRasterizer.drawDiagonalLine(posSWA.x, posSWA.y, posNWA.x, posNWA.y, blockProjectileColor);
                    }
                }
                ...
                if ((data & 0x20000) == 0x20000) { // total block (projectile)
                    ...
                    if (posSEA != null && posNEA != null) {
                        ...
                        if (posSE != null) {
                            ScreenController.gameRasterizer.drawDiagonalLine(posSE.x, posSE.y, posSEA.x, posSEA.y, blockProjectileColor);
                        }
                    ...
                    if (posSEA != null && posSWA != null) {
                        ...
                        if (posSW != null) {
                            ScreenController.gameRasterizer.drawDiagonalLine(posSW.x, posSW.y, posSWA.x, posSWA.y, blockProjectileColor);
                        }
                    }
                    if (posSWA != null && posNWA != null) {
                        ScreenController.gameRasterizer.drawDiagonalLine(posSWA.x, posSWA.y, posNWA.x, posNWA.y, blockProjectileColor);
