...
public class CreateStationaryGFXMessageHandler implements MessageHandler<CreateStationaryGFXInboundMessage> {
    ...
    public void handle(CreateStationaryGFXInboundMessage message) {
        ...
        if (x >= 0 && y >= 0 && x < 104 && y < 104) {
            y = 128 * y + 64;
            x = x * 128 + 64;
            Class40_Sub5_Sub17_Sub6 gfx = new Class40_Sub5_Sub17_Sub6(message.id, Player.worldLevel, x, y, -message.drawHeight + Class37.getFloorDrawHeight(Player.worldLevel, x, y), message.delay, MovedStatics.pulseCycle);
