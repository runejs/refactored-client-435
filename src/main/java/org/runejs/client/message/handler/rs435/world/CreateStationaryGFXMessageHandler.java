...
public class CreateStationaryGFXMessageHandler implements MessageHandler<CreateStationaryGFXInboundMessage> {
    ...
    public void handle(CreateStationaryGFXInboundMessage message) {
        ...
        if (x >= 0 && y >= 0 && x < 104 && y < 104) {
            y = 128 * y + 64;
            x = x * 128 + 64;
            SpotAnim gfx = new SpotAnim(message.id, Player.worldLevel, x, y, -message.drawHeight + Game.currentScene.getFloorDrawHeight(Player.worldLevel, x, y), message.delay, MovedStatics.pulseCycle);
